...
WEBHOOK_URL = authdata['Discord']['webhook-url']

followedTwitterIDs = ['984234517308243968', '994077432897523713'] #TSV, Tucker
        ...
        if (data['user']['id_str'] in followedTwitterIDs and 'retweeted_status' not in data):
            ...
            for userMention in data['entities']['user_mentions']:
                text = text.replace('@{0}'.format(userMention['screen_name']),
                                    '[@{0}](https://twitter.com/{0})'.format(user_mention['screen_name']))
            media_url = ''
            media_type = ''
...
if __name__ == '__main__':
    ...
    api = API(auth)
    testStatus = api.statuses_lookup([997125759931121665])[0]
    data = testStatus._json
